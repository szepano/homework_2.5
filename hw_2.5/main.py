...
def get_urls(dates_final, codes=('USD', 'EUR')):
    ...
    for code in codes:
        for date in dates_final:
            urls.append(f'https://api.nbp.pl/api/exchangerates/rates/c/{code}/{date}?format=json')
    ...
    for url in urls:
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                ...
                rates = data['rates'][0]
                date_str = datetime.strptime(rates['effectiveDate'], '%Y-%m-%d').strftime('%d.%m.%Y')
                if date_str not in exchange_dict:
                    exchange_dict[date_str] = {}
                exchange_dict[date_str][data['code']] = {
                    'sale': rates['bid'],
                    'purchase': rates['ask']
                }
    for date, currencies in exchange_dict.items():
